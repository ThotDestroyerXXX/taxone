...
@Component
public class ProjectRemovedNotificationStrategy implements NotificationStrategy<ProjectRemovedEvent> {

    @Override
    public boolean canHandle(NotificationEvent event) {
        return event instanceof ProjectRemovedEvent;
    }

    @Override
    public Notification createNotification(ProjectRemovedEvent event, User user) {
        return Notification.builder()
                .user(user)
                .message(event.getMessage())
                .notificationType(Notification.NotificationType.PROJECT_REMOVED)
                .isRead(false)
                .build();
    }
}